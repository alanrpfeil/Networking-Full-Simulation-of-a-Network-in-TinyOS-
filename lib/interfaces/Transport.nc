...
interface Transport{
   ...
   command uint8_t getServerRcvdBuffer(uint8_t fd, uint8_t* buff, uint8_t buffLen);
   
   command void sendWhisper(uint8_t fd, uint8_t flag);
   
   command uint8_t getServerNode(uint8_t fd);
   
   command uint8_t getServersNode(uint8_t fd);
   
   command void receiveMessage(uint8_t* buffer);
